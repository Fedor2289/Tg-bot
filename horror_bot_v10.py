...
def KB(stage=0):
    ...
    if stage < 2:
        ...
        k.add(KeyboardButton("🌑 Погода"),       KeyboardButton("👁 Кто ты?"))
        k.add(KeyboardButton("🩸 Игры"),         KeyboardButton("💀 /stop"))
    _kb_cache[key] = k
    return k
...
def KB_ADM():
    ...
    k.add(KeyboardButton("➖ Убрать admin'а"))
...
def KB_VIC():
    ...
    k.add(KeyboardButton("🔔 Уведомления"),          KeyboardButton("🤝 Совместный квест"))
    k.add(KeyboardButton("🔁 Авто-режим ВКЛ"),      KeyboardButton("⏹ Авто-режим ВЫКЛ"))
    k.add(KeyboardButton("🎬 Сценарий"),             KeyboardButton("⏰ Таймер-атака"))
    k.add(KeyboardButton("📊 Граф стадий"),          KeyboardButton("🏆 Лидеры страха"))
    k.add(KeyboardButton("💾 Создать сценарий"),    KeyboardButton("🗑 Удалить сценарий"))
    k.add(KeyboardButton("📋 Инфо"),               KeyboardButton("🔄 Сбросить"))
...
FINAL = [
    "ХВАТИТ", ".", "ТЫ ДУМАЛ ЧТО МОЖЕШЬ УЙТИ?",
    "💀💀💀💀💀💀💀💀💀💀", "🩸🩸🩸🩸🩸🩸🩸🩸🩸🩸",
    "👁👁👁👁👁👁👁👁👁👁👁👁",
    "Я ВСЕГДА БУДУ ЗДЕСЬ", "ВСЕГДА", "...",
    "{name}.", ".", "ОБЕРНИСЬ.", ".",
    "бот остановлен.", "но я", "—", "нет.",
]
...
PARANOIA = [
    ...
    "твой телефон светится в темноте. я вижу тебя.",
...
def on_start(msg):
    ...
    u["stopped"] = False  # Сброс stopped при /start
    send(uid,
...
def on_stop(msg):
    uid=msg.from_user.id; u=U(uid); u["stopped"]=True
    for line in FINAL:
        try:
            bot.send_message(uid, P(line,u)); time.sleep(random.uniform(0.3,1.0))
        except:
            pass
    try:
...
if __name__ == "__main__":
    ...
    print("╔══════════════════════════════════════════╗")
    print("║     👁  HORROR BOT v10.0 ЗАПУЩЕН  👁     ║")
    print("╠══════════════════════════════════════════╣")
