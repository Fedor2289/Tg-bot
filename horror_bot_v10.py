...
║  Запуск:     python horror_bot_v14.py                           ║
║  Гл. admin:  /admingo  (только ADMIN_ID)                        ║
╚══════════════════════════════════════════════════════════════════╝

ИЗМЕНЕНИЯ v14.0:
  ① Убран глобальный перезапуск (subprocess+sys.exit) — теперь
     «🔄 Перезапустить бота» делает мягкий stop_polling(),
     watchdog run_polling() поднимает его заново автоматически
  ② /start больше не блокирует receiving thread (sleep перенесён в пул)
  ③ _scheduler_loop и _random_event_loop корректно завершаются при shutdown
  ④ Убран дубликат объявления _scenarios (был на строке ~85 и ~1532)
  ⑤ Убрана лишняя очистка _kb_cache.clear() в каждом /start
  ⑥ Все bare except: заменены на except Exception:
...
def _handle_admin_inner(msg, admin_uid):
    ...
    if IS_ROOT and text == "🔄 Перезапустить бота":
        send(admin_uid, "🔄 Перезапуск polling через 2 секунды...", kb=ReplyKeyboardRemove())
        time.sleep(2)
        bot_restart()
        send(admin_uid, "✅ Polling перезапущен (процесс продолжает работу).", kb=adm_kb(admin_uid))
        return
...
if __name__ == "__main__":
    ...
    print("╔══════════════════════════════════════════╗")
    print("║     👁  HORROR BOT v14.0 ЗАПУЩЕН  👁     ║")
    print("╠══════════════════════════════════════════╣")
