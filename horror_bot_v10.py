...
║  Запуск:     python horror_bot_v15.py                           ║
║  Гл. admin:  /admingo  (личка) | /gadmin (в группе)             ║
╚══════════════════════════════════════════════════════════════════╝

ИЗМЕНЕНИЯ v15.0:
  ① Кнопки в группе — заменены на InlineKeyboard (работают без ЛС)
  ② Мафия в группе — полностью на InlineKeyboard (join/vote/ночь)
  ③ Удалена кнопка «Перезапустить бота» (не работала)
  ④ callback_query добавлен в allowed_updates
  ⑤ Единый on_callback handler вместо разрозненных
  ⑥ Групповая викторина — InlineKeyboard с вариантами ответа
  ⑦ RPG в группе — inline кнопки выбора
  ⑧ /gadmin в группе — панель управления для главного admin
  ⑨ Ночные действия мафии через InlineKeyboard в ЛС
  ⑩ Автоматический переход ночь→день после всех ночных ходов
"""
...
def _handle_admin_inner(msg, admin_uid):
    ...
    if IS_ROOT:
        ...
        if text == "👥 Группы (управление)":
            ...
            send(admin_uid,
                 "👥 ГРУППЫ:\n\n" + "\n".join(lines) +
                 "\n\nЧтобы управлять группой — добавь бота в группу и напиши /gadmin",
                 kb=KB_ADM()); return
...
def on_start(msg):
    ...
    if msg.chat.type in ("group", "supergroup"):
        ...
        # Для групп используем InlineKeyboard — ReplyKeyboard требует личного общения
        ...
        send_group(chat_id,
            ...
            f"🎮 Игры: 🎲 Число, ✏️ Виселица, 🧠 Викторина, 🗡 RPG, 🔫 Мафия\n\n"
            f"Также можно писать команды:\n"
            f"перевести [текст] | погода [город]\n"
            f"Нажми кнопку 👇", kb=k)
        return
...
def on_callback(call):
    ...
    try:
        ...
        if data.startswith("mv_"):   # mafia vote: mv_chatid_targetuid
            ...
            voted_today.add(uid)
            bot.answer_callback_query(call.id, f"Голос за {voted_name} засчитан!")
            send_group(cid, f"⚖️ {uname} голосует против {voted_name}. ({len(g['votes'])} голосов)")
            return
        ...
        if data.startswith("mcount_"):  # mafia count votes
            ...
            if uid not in g["alive"] and uid != ADMIN_ID:
                bot.answer_callback_query(call.id, "Ты не в игре."); return
        ...
        if data.startswith("mnight_"):  # mnight_chatid_playeruid_targetuid
            ...
            bot.answer_callback_query(call.id, f"✅ {action_word} {target_name}")
            try:
        ...
        if data.startswith("gg_"):
            ...
            if action == "number":    start_group_game_number(cid)
                ...
                if cid in _group_games: del _group_games[cid]; send_group(cid, "Игра остановлена.")
        ...
        # ── Управление группой от admin ──────────────────────────
        if data.startswith("gadm_") and uid == ADMIN_ID:
            parts = data.split("_")
            action = parts[1]; cid = int(parts[2])
            bot.answer_callback_query(call.id)
            if action == "horror":
                ...
                send_group(cid, f"👥 Участников в группе: {len(members)}\n" + "\n".join(lines[:30]))
...
def _handle_group_message(msg, uid, chat_id, text):
    ...
    if text in ("🎮 Групповые игры", "/games"):
    ...
    if text in ("❌ Выйти из игры", "❌ Стоп игру"):
        if chat_id in _group_games:
            ...
            send_group(chat_id, "Игра остановлена.")
    ...
    if text == "🌍 Перевести":
        send_group(chat_id, "✍️ Напиши: перевести [текст]"); return
...
def group_game_kb(chat_id):
    """InlineKeyboard для выбора игры в группе — работает без личного общения с ботом."""
    k = InlineKeyboardMarkup(row_width=2)
    k.add(
        ...
        InlineKeyboardButton("🗡 RPG-группа",   callback_data=f"gg_rpg_{chat_id}"),
        InlineKeyboardButton("🔫 Мафия",        callback_data=f"gg_mafia_{chat_id}"),
        InlineKeyboardButton("🎭 Карт. история",callback_data=f"gg_card_{chat_id}"),
        InlineKeyboardButton("❌ Стоп игру",    callback_data=f"gg_stop_{chat_id}"),
    )
    return k
...
def _mafia_night_kb(g, uid):
    ...
    for target in g["alive"]:
        ...
        # Храним chat_id в callback через g["chat_id"]
...
def proc_group_mafia(chat_id, uid, text, uname):
    ...
    if text.startswith("/vote "):
        ...
        voted_name = g["player_names"].get(target_uid, "?")
        g["votes"][uid] = target_uid
        g.setdefault("voted_today", set()).add(uid)
