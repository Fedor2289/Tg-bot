...
║  Запуск:     python horror_bot_v13.py                           ║
║  Гл. admin:  /admingo  (только ADMIN_ID)                        ║
╚══════════════════════════════════════════════════════════════════╝

ИЗМЕНЕНИЯ v13.0:
  ① Починен перезапуск бота — subprocess.Popen + sys.exit(0)
  ② Групповые игры теперь СОВМЕСТНЫЕ — голосованием выбирают действие
  ③ Мафия в группах — inline-кнопки, ночные действия в ЛС
  ④ Мафия в ЛС — создай лобби, друзья присоединяются по номеру
  ⑤ Карточная история — выбор персонажа влияет на сценарий
  ⑥ Групповая карточная история — каждый выбирает персонажа,
     голосуют за действия, бонус-ходы по классу персонажа
  ⑦ Исправлены баги: имена в группах, handle_group_message,
     duplicate game keys, GALLOWS out-of-range, trivia answered set type

ИЗМЕНЕНИЯ v12.0:
  ① Редактирование данных пользователя
  ② Перезапуск бота (починен в v13)
  ③ Детальная карточка пользователя
  ④ Работа в группах
  ⑤ Групповые игры
  ⑥ Заморозка уровня ужаса
"""
...
STAGE_SEC        = 120             # секунд между стадиями
...
# ── v11 хранилища ──────────────────────────────────────────
_scenarios  = {}  # name → [(text, delay), ...]   — сохранённые сценарии
...
def KB(stage=0):
    ...
    if stage < 2:
        ...
        k.add(KeyboardButton("🌍 Перевести"),    KeyboardButton("🌑 Погода"))
        k.add(KeyboardButton("🎮 Игры"),         KeyboardButton("🎲 Угадай"))
        k.add(KeyboardButton("🔮 Предсказание"), KeyboardButton("👁 ..."))
        k.add(KeyboardButton("🗓 Задание дня"),  KeyboardButton("🏆 Мой рейтинг"))
    else:
        # На высоких стадиях кнопка "Перевести" остаётся всегда
        k.add(KeyboardButton("🌍 Перевести"),    KeyboardButton("🌑 Погода"))
        k.add(KeyboardButton("👁 Кто ты?"),      KeyboardButton("🩸 Игры"))
        k.add(KeyboardButton("🗓 Задание дня"),  KeyboardButton("🏆 Мой рейтинг"))
...
def KB_ADM():
    ...
    k.add(KeyboardButton("➖ Убрать admin'а"),     KeyboardButton("🔄 Перезапустить бота"))
...
def fake_ban_sequence(uid):
    ...
    def _run():
        ...
        for i in range(9, 0, -1):
            time.sleep(1); send(uid, str(i) + "...")
        time.sleep(1); send(uid, "🗑 Чат удалён.")
        time.sleep(3); send(uid, "...")
        time.sleep(2); send(uid, "шучу.", kb=KB(u["stage"]))
        time.sleep(1); send(uid, P("или нет, {name}?", u))
    _pool.submit(_run)
...
def death_timer(uid, seconds=30):
    ...
    def _run():
        send(uid, P("💀 {name}.", u))
        time.sleep(1); send(uid, "ты уже мёртв.")
        time.sleep(1); send(uid, "просто ещё не знаешь.")
        time.sleep(1); send(uid, f"осталось {seconds} секунд.")
        time.sleep(2)
        step = max(1, seconds // 8); left = seconds
        while left > 0:
            if u.get("stopped"): return
            time.sleep(step); left = max(0, left - step)
            if left > 0: send(uid, f"⏳ {left}...")
            else:
                send(uid, "0")
                time.sleep(1)
                for p in [".", "..", "...", "....", "...........", "ОБЕРНИСЬ"]:
                    send(uid, p); time.sleep(0.8)
                send_screamer(uid)
                time.sleep(1)
...
def heartbeat_event(uid):
    ...
    def _run():
        ...
        send(uid, "БУМ. БУМ.")
        time.sleep(0.5)
        send(uid, "БУМ. БУМ. БУМ.")
        time.sleep(0.4)
        send(uid, "Б У М . Б У М . Б У М .")
        time.sleep(3)
        send(uid, "...")
        time.sleep(2)
...
def _scheduler_loop():
    ...
    while True:
        now = time.time()
        fired = []
        for attack in _scheduled_attacks:
...
PANIC = [
    ("ЧТО-ТО НЕ ТАК", 0.8), ("ЧТО-ТО НЕ ТАК", 0.6), ("ЧТО-ТО НЕ ТАК", 0.5),
    ("ТЫ СЛЫШИШЬ ЭТО?", 1.0), ("🩸🩸🩸🩸🩸🩸🩸🩸🩸🩸", 0.8),
    ("{name} ОБЕРНИСЬ", 1.0), ("💀💀💀💀💀💀💀💀💀💀", 0.7),
    ("{name} ОБЕРНИСЬ СЕЙЧАС", 0.8), ("👁👁👁👁👁👁👁👁👁👁", 0.6),
    ("ОН УЖЕ В КОМНАТЕ", 1.2), ("🩸💀👁🩸💀👁🩸💀👁🩸", 0.8),
    ("ПОЗДНО", 1.5), (".", 1.0), ("..", 0.8), ("...", 1.0),
    ("прости. я пошутил.", 2.0), ("или нет.", 1.5), ("👁", 0),
]
TRAP_DIALOG = [
    ...
    if stage == 1:
        ...
        if roll < 0.18:
            for p in [P(c, u) for c in random.choice(CHAINS)]:
                send(uid, p); time.sleep(random.uniform(0.6, 2.2))
        elif roll < 0.35:
        ...
        if roll < 0.12:
            for p in [P(c, u) for c in random.choice(CHAINS)]:
                send(uid, p); time.sleep(random.uniform(0.2, 1.0))
        elif roll < 0.24:
            send_screamer(uid)
            ...
            for p in [P(c, u) for c in random.choice(CHAINS)]:
                send(uid, p); time.sleep(random.uniform(0.25, 0.9))
            send_screamer(uid)
            ...
            for p in [P(c, u) for c in random.choice(CHAINS)]:
                send(uid, p); time.sleep(random.uniform(0.15, 0.7))
            send_screamer(uid)
            time.sleep(random.uniform(1, 3))
...
def horror_loop(uid):
    ...
    while not u.get("stopped"):
        ...
        stage = u["stage"]
        dn    = dnight(); n = night()
        if   stage <= 1: sl = random.randint(70, 180)
        elif stage == 2: sl = random.randint(40, 110)
        elif stage == 3: sl = random.randint(18, 60)
        else:            sl = random.randint(7, 30) if dn else (random.randint(9, 38) if n else random.randint(12, 48))
...
def stage_loop(uid):
    ...
    while not u.get("stopped"):
        ...
        u["stage"] = cur + 1
        new = u["stage"]
        # Сбрасываем кеш клавиатур при смене стадии
        _kb_cache.clear()
...
def proc_game(uid, text):
    ...
    if gm == "riddle":
        ...
        if tl.strip()==g["answer"]:
            u["score"]=u.get("score",0)+5; send(uid, f"✅ Правильно! +5  🏆 {u['score']}", kb=kb)
        else:
            send(uid, f"❌ Ответ: {g['answer']}", kb=kb)
        return True
    ...
    if not tu.get("horror_active"):
        ...
        def _b():
            for _ in range(random.randint(7,11)):
                horror_tick(tid); time.sleep(random.uniform(0.6,2.2))
        _pool.submit(_b)
...
def _handle_admin_inner(msg, admin_uid):
    ...
    # ── Root-only: перезапуск ────────────────────────────────
    if IS_ROOT and text == "🔄 Перезапустить бота":
        send(admin_uid, "🔄 Перезапуск бота через 2 секунды...", kb=ReplyKeyboardRemove())
        time.sleep(2)
        bot_restart()
        return
...
def on_start(msg):
    ...
    if msg.chat.type in ("group", "supergroup"):
        ...
        k = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
        k.add(KeyboardButton("🎮 Групповые игры"), KeyboardButton("🏆 Рейтинг"))
        k.add(KeyboardButton("🌍 Перевести"),      KeyboardButton("🌤 Погода"))
        send_group(chat_id,
            f"👁 Horror Bot подключился к группе!\n\n"
            f"Участник: {msg.from_user.first_name}\n\n"
            f"Игры: 🎲 Число, ✏️ Виселица, 🧠 Викторина, 🗡 RPG\n"
            f"Доступны командами ниже 👇", kb=k)
        return
    ...
    # Небольшая пауза — даём потокам хоррора/стадий заметить флаг
    time.sleep(1.5)
    ...
    for pid in [k for k, v in list(_active_polls.items()) if v.get("uid") == uid]:
        _active_polls.pop(pid, None)
    _kb_cache.clear()
...
def on_mafia_callback(call):
    """Обрабатывает inline-кнопки мафии в группе."""
    try:
        ...
        if data.startswith("mafia_join_"):
            ...
            if not g or g.get("state") != "lobby":
                bot.answer_callback_query(call.id, "Игра уже началась или завершена.")
                return
            if uid in g["players"]:
            ...
            bot.answer_callback_query(call.id, f"✅ Ты вступил в игру!")
            send_group(chat_id, f"✅ {uname} присоединился к мафии! Игроков: {len(g['players'])}")

        elif data.startswith("mafia_start_"):
            chat_id = int(data.split("_")[2])
            g = _group_mafia.get(chat_id)
            if not g or g.get("state") != "lobby":
...
def _on_text_inner(msg):
    ...
    if text=="🎭 Загадка" or any(w in tl for w in ["загадка","загадай"]):
        ...
        games[uid]={"game":"riddle","answer":a}
...
def _handle_group_message(msg, uid, chat_id, text):
    ...
    if text == "🎮 Групповые игры":
        kb = group_game_kb(chat_id)
        send_group(chat_id, "🎮 Выбери игру для группы:\n\n🎲 Число, ✏️ Виселица, 🧠 Викторина, 🗡 RPG\n🔫 Мафия, 🎭 Карточная история", kb=kb)
        return
    ...
    if text == "🗡 RPG-группа":
        ...
        send_group(chat_id, scene["text"], kb=kb); return
    if text == "🔫 Мафия":
    ...
    if text == "🌍 Перевести":
        send_group(chat_id, "✍️ Напиши текст после команды: перевести [текст]"); return
    if tl.startswith("перевести "):
        src = text[10:].strip()
        res = translate(src, "ru|en")
        send_group(chat_id, f"🌍 {res}" if res else "❌ Не удалось перевести."); return

    if text == "🌤 Погода":
        send_group(chat_id, "Напиши: погода [город]"); return
...
def group_game_kb(chat_id):
    k = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    k.add(KeyboardButton("🎲 Угадай число (группа)"), KeyboardButton("✏️ Виселица (группа)"))
    k.add(KeyboardButton("🧠 Викторина (группа)"),    KeyboardButton("🗡 RPG-группа"))
    k.add(KeyboardButton("🔫 Мафия"),                 KeyboardButton("🎭 Карточная история"))
    k.add(KeyboardButton("❌ Выйти из игры"))
    return k
...
def start_group_mafia(chat_id):
    ...
    if chat_id in _group_games:
        send_group(chat_id, "⚠️ Сначала заверши текущую игру: ❌ Выйти из игры")
        return
    _group_mafia[chat_id] = {
        ...
        "phase": "day",  # day / night
        "votes": {},
        "night_actions": {},  # шериф/доктор ходы
        "mafia_target": None,
        "day_num": 0,
    }
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("✅ Участвую", callback_data=f"mafia_join_{chat_id}"))
    kb.add(InlineKeyboardButton("▶️ Начать игру", callback_data=f"mafia_start_{chat_id}"))
    send_group(chat_id,
        "🔫 МАФИЯ!\n\n"
...
def _mafia_start_game(chat_id):
    ...
    send_group(chat_id,
        f"🎮 МАФИЯ НАЧИНАЕТСЯ!\n\nИгроки ({len(players)}):\n{names_list}\n\n"
        f"☀️ ДЕНЬ 1\n\nОбсуждайте кто мафия! Голосуйте командой:\n/vote @имя",
        kb=_mafia_day_kb(g))
...
def _mafia_day_kb(g):
    ...
    for uid in g["alive"]:
        name = g["player_names"].get(uid, f"ID:{uid}")
        k.add(KeyboardButton(f"⚖️ {name}"))
    k.add(KeyboardButton("🏁 Подсчитать голоса"))
    k.add(KeyboardButton("❌ Выйти из игры"))
    return k
...
def _mafia_end(chat_id, winner):
    ...
    send_group(chat_id, msg, kb=group_game_kb(chat_id))
...
def proc_group_mafia(chat_id, uid, text, uname):
    ...
    if phase == "day":
        # Голосование за выбывшего
        if text.startswith("⚖️ "):
            target_name = text[2:].strip()
            # Ищем uid по имени
            target_uid = None
            for p in g["alive"]:
                if g["player_names"].get(p, "") == target_name:
                    target_uid = p; break
            if not target_uid:
                return True
            if uid == target_uid:
                send_group(chat_id, f"⚠️ {uname}, нельзя голосовать за себя!")
                return True
            g["votes"][uid] = target_uid
            voted_name = g["player_names"].get(target_uid, "?")
            send_group(chat_id, f"⚖️ {uname} голосует против {voted_name}. ({len(g['votes'])} голосов)")
            return True

        if text == "🏁 Подсчитать голоса":
            if not g["votes"]:
                send_group(chat_id, "Никто не проголосовал!"); return True
            # Считаем голоса
            count = {}
            ...
            g["votes"] = {}
            send_group(chat_id,
                f"⚖️ Большинство голосов: {elim_name}\n"
                f"Его роль: {MAFIA_ROLES.get(elim_role,'?').split(chr(10))[0]}\n\n"
                f"{'🔫 Это был мафиози!' if elim_role=='мафия' else '😢 Мирный житель выбыл...'}")
            # Проверка победы
            winner = _mafia_check_win(chat_id)
            if winner:
                _mafia_end(chat_id, winner); return True
            # Переходим к ночи
            g["phase"] = "night"
            g["night_actions"] = {}
            g["mafia_target"] = None
            send_group(chat_id, "🌙 НОЧЬ\n\nВсе закрыли глаза...\nМафия, шериф и доктор — действуйте в личке!")
            # Уведомляем игроков с особыми ролями
            for p in g["alive"]:
                role = g["roles"].get(p)
                if role in ("мафия", "шериф", "доктор"):
                    ...
                    try:
                        ...
                        _safe_call(bot.send_message, p,
                            ...
                            f"Напиши мне имя игрока которого хочешь {action}:",
        ...
        send_group(chat_id, "🌙 Сейчас ночь. Ночные действия — в личных сообщениях.")
        return True
...
def _mafia_announce_day(lobby_id):
    ...
    day_text = (
        f"☀️ ДЕНЬ {g['day_num']}\n\n"
        f"Живые игроки:\n{alive_names}\n\n"
        f"Голосуй: напиши имя игрока которого хочешь устранить.\n"
        f"Или «пропустить» чтобы не голосовать."
    )
    for uid in g["alive"]:
        send(uid, day_text)
...
def _mafia_announce_night(lobby_id):
    ...
    for uid in g["alive"]:
        ...
        if role == "мирный":
            ...
            alive_peaceful = [g["player_names"].get(p,"?") for p in g["alive"] if g["roles"].get(p) != "мафия"]
            send(uid, f"🌙 НОЧЬ — ТЫ МАФИЯ\n\nЖивые мирные:\n" + "\n".join(alive_peaceful) + "\n\nНапиши имя кого убить:")
        elif role == "шериф":
            alive_others = [g["player_names"].get(p,"?") for p in g["alive"] if p != uid]
            send(uid, f"🌙 НОЧЬ — ТЫ ШЕРИФ\n\nНапиши имя кого проверить:\n" + "\n".join(alive_others))
        elif role == "доктор":
            alive_others = [g["player_names"].get(p,"?") for p in g["alive"]]
            send(uid, f"🌙 НОЧЬ — ТЫ ДОКТОР\n\nНапиши имя кого вылечить:\n" + "\n".join(alive_others))
...
def proc_group_game(chat_id, uid, text):
    ...
    if text == "❌ Выйти из игры":
        del _group_games[chat_id]
        send_group(chat_id, "🚫 Игра завершена.")
        return True
    ...
    if gm == "number":
        if tl.lstrip("-").isdigit():
            ...
            if guess == num:
                ...
                del _group_games[chat_id]
                send_group(chat_id, f"😔 Попытки закончились! Число было: {num}")
            else:
    ...
    if gm == "hangman":
        if len(tl) == 1 and tl.isalpha():
            ...
            if "_" not in display:
                ...
                del _group_games[chat_id]
                send_group(chat_id, f"{icon} Слово: {word.upper()}")
            else:
...
def bot_restart():
    """Полный перезапуск процесса бота (только главный admin)."""
    import subprocess
    _shutdown.set()
    try:
        bot.stop_polling()
        ...
        pass
    time.sleep(1)
    # subprocess.Popen запускает новый процесс и сразу выходим из текущего
...
if __name__ == "__main__":
    ...
    print("╔══════════════════════════════════════════╗")
    print("║     👁  HORROR BOT v12.0 ЗАПУЩЕН  👁     ║")
    print("╠══════════════════════════════════════════╣")
