...
║  Запуск:     python horror_bot_v16.py                           ║
║  Гл. admin:  /admingo  (личка)                                  ║
╚══════════════════════════════════════════════════════════════════╝

ИЗМЕНЕНИЯ v16.0 (исправления):
  ① МАФИЯ: голосование анонимное — никто не видит кто за кого голосовал
  ② МАФИЯ: ночные действия приватные — только confirm в ЛС
  ③ МАФИЯ: автоподсчёт голосов когда все проголосовали
  ④ МАФИЯ: ничья при голосовании — никто не выбывает
  ⑤ МАФИЯ в ЛС: полностью на InlineKeyboard (день+ночь)
  ⑥ УПРАВЛЕНИЕ ГРУППАМИ: полностью из ЛС бота в /admingo
  ⑦ ГРУППА: постоянные Reply-кнопки (всегда видны внизу)
  ⑧ gadm_ callbacks работают из ЛС а не только из группы
  ⑨ Список участников группы — только в ЛС admin'у
  ⑩ Оптимизация: кеш клавиатур, thread-safe голосование
"""
...
def on_callback(call):
    ...
    try:
        ...
        # ── Управление группой от admin (из ЛС или из группы) ──────────────────────
...
def group_reply_kb():
    """Постоянные кнопки Reply-клавиатуры для группы (видны всегда внизу)."""
    k = ReplyKeyboardMarkup(resize_keyboard=True, row_width=3)
    k.add(
        KeyboardButton("🎮 Групповые игры"),
