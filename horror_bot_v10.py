...
║  Запуск:     python horror_bot_v12.py                           ║
║  Гл. admin:  /admingo  (только ADMIN_ID)                        ║
╚══════════════════════════════════════════════════════════════════╝

ИЗМЕНЕНИЯ v12.0:
  ① Редактирование данных пользователя — админ меняет имя, город, страх и т.д.
  ② Перезапуск бота прямо из Telegram (только главный admin)
  ③ Детальная карточка пользователя с кнопкой 📋 Инфо (расширенная)
  ④ Работа в группах — /start, /stop, игры, хоррор-режим
  ⑤ Групповые игры — виселица, угадай число, викторина, совместный RPG
  ⑥ Улучшенная RPG — случайные события, встречи, развилки
  ⑦ Заморозка уровня ужаса — зафиксировать стадию на N минут
  ⑧ Исправлены все обнаруженные ошибки

ИЗМЕНЕНИЯ v11.0:
  ① Режим экзорциста — нарастающий ритуал на 10 минут
  ② Фейк-трансляция — бот «видит» жертву в реальном времени
  ③ GPS-трекинг — бот описывает маршрут жертвы
  ④ Взлом Wi-Fi — бот «подключился» к сети жертвы
  ⑤ Фейк-уведомления — ВКонтакте, WhatsApp, банк
  ⑥ Ежедневные задания — страшные квесты на каждый день
  ⑦ Таблица лидеров страха — рейтинг жертв по баллам
  ⑧ Совместный квест — 2 жертвы решают загадки вместе
  ⑨ Случайные события — новый хоррор каждые N часов
  ⑩ Авто-режим — бот сам выбирает атаки по поведению жертвы
  ⑪ Сценарии — сохранять и запускать цепочки атак
  ⑫ Таймер-атака — запланировать хоррор на точное время
  ⑬ Граф стадий — история прогресса каждой жертвы

ИЗМЕНЕНИЯ v10.0 (предыдущая версия):
  ① Гифки из папки gif/ — автоматически подхватывает новые файлы
  ② Фейковая геолокация — бот отправляет «координаты» жертвы
  ③ Фейковый скан телефона — модель, батарея, Wi-Fi, камера
  ④ Иллюзия других пользователей — фейковые сообщения от «людей»
  ⑤ Фейковое чтение файлов — список файлов с устройства
  ⑥ Умное эхо истории — «3 минуты назад ты писал: ...»
  ⑦ 03:00 режим — особые сообщения в самую страшную ночь
  ⑧ Потеря сигнала — фейковые помехи соединения
  ⑨ Фейковые системные уведомления Telegram Security
  ⑩ Новые кнопки в admin-панели: 📍 Геолокация, 📱 Скан, 👥 Призраки,
     📂 Файлы, 💬 Эхо, 📡 Потеря сигнала, 🕒 Режим 3AM
  ⑪ Исправлены все известные баги v9
"""
...
def proc_game(uid, text):
    ...
    if gm == "hangman":
        if len(tl)==1 and tl.isalpha():
            ...
            icon = GALLOWS[min(6-g["attempts"],6)]
            if "_" not in display:
...
def _handle_group_message(msg, uid, chat_id, text):
    ...
    if text == "🎮 Групповые игры":
        ...
        kb.add(KeyboardButton("🎲 Угадай число (группа)"), KeyboardButton("✏️ Виселица (группа)"))
        kb.add(KeyboardButton("🧠 Викторина (группа)"),    KeyboardButton("🗡 RPG-группа"))
